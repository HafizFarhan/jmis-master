...
ol.control.Search = function(options) {
  ...
  var doSearch = function(e) {
    ...
    // Complete Dropdown Code -- START
    var searchCatKeys = ["Tracks", "COIs", "Drawings", "Preliminary Reports"];
    var searchCatValues = ["trackNumber", "coiNumber", "name", "prNumber"];
    var drpSearchCat = document.createElement("select");
    drpSearchCat.id = "drpSearchCat";
    for (var i = 0; i < searchCatKeys.length; i++) {
        var option = document.createElement("option");
        option.value = searchCatValues[i];
        option.text = searchCatKeys[i];
        drpSearchCat.appendChild(option);
    }
    drpSearchCat.onchange = function () {
        if (drpSearchCat.options[drpSearchCat.selectedIndex].text === 'Drawings') {
            search.setSource(drawingVectorSource);
        }
        if (drpSearchCat.options[drpSearchCat.selectedIndex].text === 'COIs') {
            search.setSource(coiVectorSource);
        }
        if (drpSearchCat.options[drpSearchCat.selectedIndex].text === 'Preliminary Reports') {
            search.setSource(prVectorSource);
        }
        if (drpSearchCat.options[drpSearchCat.selectedIndex].text === 'Tracks') {
            search.setSource(localAISVectorSource);
        }
        search.set('property', drpSearchCat.value);
    };
    // Complete Dropdown Code -- END
  ...
  if (!options.noCollapse) {
    input.addEventListener('blur', function() {
      setTimeout(function(){ 
          if (input !== document.activeElement && drpSearchCat !== document.activeElement) {
    ...
    element.appendChild(drpSearchCat);
